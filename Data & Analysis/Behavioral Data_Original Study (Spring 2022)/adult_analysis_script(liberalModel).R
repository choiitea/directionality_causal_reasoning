...
for(i in 1:nrow(D)){
  if(D$ordered_contrDir_choice[i]==D$test_contrDir_choice[i]){
    ...
    D$control_choice[i] = 0 #chose inverse but not order_con
  }else if(D$test_contrDir_choice[i]=="right"&&D$ordered_contrDir_choice[i]=="left"){
    D$control_choice[i] = 0 #chose inverse but not order_con
  }else{
    D$control_choice[i] = 0 #neither
  }
}
...
baseline_control_ordered_prob = table(D$control_choice)[[2]]/(table(D$control_choice)[[1]]+table(D$control_choice)[[2]])
...
baseline_control_both_prob = table(D$control_choice)[[1]]/(table(D$control_choice)[[1]]+table(D$control_choice)[[2]])
...
D$direction_choice = revalue(x = as.factor(D$direction_choice), 
                             c("0" = "Both or Other", "1"="SOC-Consistent"))
D$direction_choice = factor(D$direction_choice, levels = c("Both or Other", "SOC-Consistent"))

D$direction_choice = relevel(D$direction_choice, ref="SOC-Consistent")
glm_direction_choice = glm(direction_choice~1, data = D, family = "binomial")
summary(glm_direction_choice)

# control_choice_main_analysis
D$control_choice = revalue(x = as.factor(D$control_choice), 
                           c("0" = "Both or Other", "1"="Order-Consistent"))
D$control_choice = factor(D$control_choice, levels = c("Both or Other", "Order-Consistent"))
D$control_choice = relevel(D$control_choice, ref="Order-Consistent")
glm_control_choice = glm(control_choice ~ 1, data = D, family = "binomial")
